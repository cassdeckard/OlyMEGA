...
* **In Hand:**
    ...
    * B0505S-1W DC-DC Isolation Module
* **Delayed in Transit:**
...
* **Isolation Architecture:**
    * Power path:
        ...
        * **B0505S-1W DC-DC Isolator**
        * MH-M18 VCC/GND input
    * Keep the B0505S grounds completely isolated:
        * Pin 1: Input Ground
        * Pin 3: Output Ground
    * Purpose: prevent data noise from bleeding into the audio path.
* **Front-Panel KEY Control:**
    * Install a normally-open momentary pushbutton.
    * Wire the switch between:
        * MH-M18 Pin 1 (`KEY`)
        * MH-M18 Pin 4 (`GND`)
            * This is the module-side isolated ground.
            * It connects to B0505S Pin 3 (`Output Ground`).
    * Use a direct 0Ω switch closure for this control.
    * Keep the switch and its return entirely on the isolated output side.
...
--------------------------------------------------------------------------------
                           ...
                           [Hidden Nook 5V Adapter]
                              ...
                              +5V and input ground
                                        │
                                        ▼
                        [B0505S Input: Pins 1 and 2]
                                        ║
                                        ║
                          Galvanic isolation barrier
                                        ║
                                        ║
                        [B0505S Output: Pins 3 and 4]
                                        │
                           Isolated +5V and ground
                                        │
                                        ▼
...
* **Preferred Architecture:**
    ...
    * Switch only the isolated +5V supply to MH-M18 `VCC`.
* **Power Integration:**
    * Keep the Nook adapter powered from the Yamaha's switched AC outlet.
    * Keep the B0505S energized whenever the receiver is on.
    * Place the high-side switch between:
        * B0505S Pin 4 (`+5V Output`)
        * MH-M18 Pin 3 (`VCC`)
...
* **Selector-Tap Constraints:**
    ...
    * Keep the entire decoder and load-switch control circuit on the
        module-side/Yamaha-ground side of the B0505S isolation barrier.
...
--------------------------------------------------------------------------------
 [Yamaha Selector State] ─> [Buffer / Decoder] ─> [Switch Enable]
                                                        │
                                                        ▼
 [Nook 5V] ─> [B0505S] ─> [Isolated +5V] ─> [High-Side VCC Switch]
                                                        │
                                                        ▼
                                                   [MH-M18 VCC]
